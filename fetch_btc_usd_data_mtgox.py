...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fetch_data_mtgox.log'),
        logging.StreamHandler()
    ]
)

# Define the time range for MtGox (early exchange, 2010–2012)
start_date = pd.to_datetime('2010-07-17 00:00:00', utc=True)  # Earliest possible date
end_date = pd.to_datetime('2012-06-01 00:00:00', utc=True)  # Stop before Coinbase data
...
fsym = "BTC"
tsym = "USD"
exchange = "MtGox"  # Switch to MtGox
limit = 2000  # Max allowed by CryptoCompare per request
chunk_size = limit
output_path = 'btc_usd_historical_mtgox.csv'
...
max_consecutive_sparse = 15  # Allow more gaps
...
if all_data:
    ...
    df = df.set_index('timestamp').reindex(
        ...
        method='ffill'
    ).reset_index()  # Fill gaps in timestamps
