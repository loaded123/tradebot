...
def calculate_rsi(prices, window=14, volatility_threshold=0.02):
    ...
    price_volatility = prices.pct_change().rolling(window=24).std()  # 24-hour rolling std
    ...
    delta = prices.diff()
    gain = (delta.where(delta > 0, 0)).rolling(window=dynamic_window).mean()
    loss = (-delta.where(delta < 0, 0)).rolling(window=dynamic_window).mean()
    rs = gain / loss
    return 100 - (100 / (1 + rs))
...
def calculate_macd(prices, fast=12, slow=26, signal=9, volatility_threshold=0.02):
    ...
    price_volatility = prices.pct_change().rolling(window=24).std()
    dynamic_fast = fast
    dynamic_slow = slow
    ...
    return macd - signal_line, signal_line

def calculate_atr(high, low, close, period=14, volatility_threshold=0.02):
    """Calculate Average True Range (ATR) with dynamic window size based on volatility."""
    # Calculate price volatility (e.g., 1-hour returns standard deviation)
    price_volatility = close.pct_change().rolling(window=24).std()  # 24-hour rolling std
    ...
    return tr.rolling(window=dynamic_window).mean().bfill().fillna(500.0)  # Backfill and default to 500.0

def calculate_sentiment_score(symbol="BTC/USD", source="twitter"):
    """Calculate market sentiment score from Twitter/X or news (simplified example)."""
    try:
        # Mock API call (replace with actual API like Twitter/X or Reddit)
        response = requests.get(f"https://api.example.com/{symbol}/sentiment?source={source}")
...
def get_onchain_metrics(symbol="BTC"):
    """Fetch on-chain metrics (e.g., whale transactions, hash rate) (simplified example)."""
    try:
        # Mock API call (replace with Glassnode/CryptoQuant API)
        response = requests.get(f"https://api.glassnode.com/{symbol}/metrics")
        metrics = response.json()
        whale_moves = metrics.get('whale_transactions', 0)  # Number of large transactions
        hash_rate = metrics.get('hash_rate', 0)  # Network health
